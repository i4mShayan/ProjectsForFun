...
ans[6] = [['از وقفه ایجاد شده راضی هستید و از آن استقبال می کنید'],['بسختی ناراحت می شوید'],['با پوزخند کوچک'],['حالتی بینابین این ۲ حالت ایجاد می شود']]
...
def fin(update, context):
    ...
    update.send_message(admin, user['username'] + result)
